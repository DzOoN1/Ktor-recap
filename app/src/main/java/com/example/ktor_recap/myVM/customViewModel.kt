...
class customViewModel(private val repository: MyRepository) : ViewModel() {
 ...
 var listAllNotesVM: MutableList<Note> by mutableStateOf(mutableListOf())
    suspend fun getHelloWorld() {

        noteTitle =  repository.getHelloWorld()
...
//        client.close()
    ...
    suspend fun getRandomNote() {
       val rNote = repository.getRandomNote()
        noteId = rNote.id
        noteTitle = rNote.title
        noteNumberOfPages = rNote.numberOfPages
//        val client = HttpClient(CIO) {
...
//        client.close()
    ...
    suspend fun getAllNotes() {
       listAllNotesVM = repository.getAllNotes()
//        val client = HttpClient(CIO) {
